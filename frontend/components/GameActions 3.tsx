...
export default function GameActions() {
  ...
  const handleDeal = async () => {
    setLoading('deal')
    try {
      // Show deck shuffling info on first deal
      if (!handDealt) {
        toast.success('🃏 7-deck shoe being dealt. Auto-reshuffle at 67% penetration.', {
          duration: 4000,
          icon: '🎰',
        })
      }
  ...
  const handleCashOut = async () => {
    setLoading('cashout')
    try {
      // Reset local state first (doesn't require backend)
      cashOut()
      toast.success('Tokens cashed out successfully')

      // Attempt backend call but don't fail if it doesn't exist
      try {
        await postJSON('/api/game/cashout', {
          handId,
        })
      } catch (err) {
        // Backend endpoint may not exist or throw error, that's okay
        console.log('Backend cashout endpoint not available or failed, continuing with redirect')
      }

      // Always navigate to checkin
      router.push('/checkin')
    } finally {
      setLoading(null)
    }
